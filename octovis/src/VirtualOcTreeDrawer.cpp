...
namespace octomap {
  ...
  VirtualOcTreeDrawer::VirtualOcTreeDrawer()
    : OcTreeDrawer() {
  }
  ...
  void VirtualOcTreeDrawer::setOcTree(const AbstractOcTree& tree_pnt,
                                    const octomap::pose6d& origin_,
                                    int map_id_) {

    const VirtualOcTree& tree = ((const VirtualOcTree&) tree_pnt);
    ...
    for(VirtualOcTree::tree_iterator it = tree.begin_tree(this->m_max_tree_depth),
    ...
    for(VirtualOcTree::tree_iterator it = tree.begin_tree(this->m_max_tree_depth),
      ...
      if (it.isLeaf()) { // voxels for leaf nodes
        ...
        if (tree.isNodeOccupied(*it)){ // occupied voxels
          // Draw predicted voxels in red and real voxels in blue.
          uint8_t r = 255u;
          uint8_t g = 0u;
          uint8_t b = 0u;
          ...
          if (tree.isNodeAtThreshold(*it)) {
            ...
            color_idx_occupied_thres =  setCubeColorRGBA(r, g, b,
            ...
            color_idx_occupied = setCubeColorRGBA(r, g, b,
                                                  (unsigned char)(it->getOccupancy() * 255.),
